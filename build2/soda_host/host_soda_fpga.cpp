...
int FPGA_SODA::fpga_init(const std::string& xclbin_path, const unsigned int device_index) {
    ...
    inA_bo  = xrt::bo(device, A_BYTES, soda_kernel.group_id(0));
    outB_bo = xrt::bo(device, B_BYTES, soda_kernel.group_id(1));

    inA_host_ptr  = inA_bo.map<float*>();
    outB_host_ptr = outB_bo.map<float*>();

    std::cout << "Mapped pointers: A=" << (void*)inA_host_ptr
              << " B=" << (void*)outB_host_ptr << std::endl;

    if (inA_host_ptr == nullptr || outB_host_ptr == nullptr) {
        std::cerr << "Error: Failed to map XRT Buffer Objects to host memory!" << std::endl;
        return -1; 
    }

    run_soda = xrt::run(soda_kernel);
    run_soda.set_arg(0, inA_bo);
    run_soda.set_arg(1, outB_bo);
...
void FPGA_SODA::run() {
    ...
    outB_bo.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
...
float* FPGA_SODA::get_inA_ptr() { return inA_host_ptr; }
float* FPGA_SODA::get_outB_ptr() { return outB_host_ptr; }